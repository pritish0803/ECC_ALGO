...
cout << "Your values do not satisfied the condition" << endl; cout << "Please put values again" << endl;
check = true;
}
} while (check);
cout << "----------------------------------------------------------" << endl;
...
<< Cy << "))" << endl;
cout << "||-----------------------------------------||" << endl;
cout << "Person2 receive the message and start decrypting" << endl; decryption(Cx, Cy, n, a, p, x1, y1, Mx, My);
cout << "Decrypted message is (" << Mx << "," << My << ")" << endl;
}
bool CheckIsPrime(long int num) {
if(num < 2) return false; long int i = 2;
while(i <= num / 2)
{
...
return num1 * num2; }
bool CheckCoPrime (long int num1, long int num2) { long int lowest;
if (num1 > num2) lowest = num2;
else lowest = num1;
long int i = 2;
bool coprime = true; while (i < lowest) {
...
void EncDecStr (long int e, long int n) {
 ...
 char *str = new char[1000]; char *str1 = new char[1000]; cout << "\nEnter a string: ";
cin >> str;
...
void delay(){
 ...
 cout<<"Delay Rsa ,Ecc  "<<endl; cout<<"Number Of Node(n) | RSA | ECC "<<endl; cout<<"--------------------------------------------- "<<endl;
int x,y;
